...
$dashboard = file_get_contents('templates/account/joined_game.html');
...
$request = array();
$request['request'] = array('userId'=>$Id,'orderBy'=>"ID");
...
for($i=0;$i<count($result_array['data']);$i++){
	if($result_array['data'][$i]['joined_status']=='Yes'){
...
if($game_out==''){
	$game_out='<div style="text-align: center;">There are no joined contests.</div>';
}
