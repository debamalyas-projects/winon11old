...
$dashboard = file_get_contents('templates/account/contest_over.html');
...
$request = array();
$request['query'] = "SELECT * FROM `contests` WHERE `ClosingStatus`= '3' ORDER BY `ID` DESC";
...
for($i=0;$i<count($result_array['data']);$i++){
	...
	if($con>0)
	{
		// Display customer won amount

		$request = array();
		$request['query'] = "SELECT * FROM `rank` WHERE `CustomerID`= '".$Id."' AND `ContestID`='".$result_array['data'][$i]['ID']."' AND `Status`='1'";
		...
		$total_won_amount=0;
		for($m=0;$m<count($result_array1['data']);$m++){
			$total_won_amount = $total_won_amount+ $result_array1['data'][$m]['won'];
		}

		// Display customer won amount
		$game=file_get_contents('templates/account/contest_over_list.html');
		...
		$tags_arr=array(
				...
				'*{total_win_amount}*' => $total_won_amount
				);
...
if($game_out==''){
	$game_out='<div style="text-align: center;">There are no compleated contests.</div>';
}
